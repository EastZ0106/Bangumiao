...
#[tauri::command]
pub fn wipe_all_data(state: State<'_, AppState>) -> Result<String, String> {
    let app = state.inner();
    // Take a strong lock and delete everything
    {
        ...
        let ep_count: i32 = conn
            ...
            .unwrap_or(0);
        let sub_count: i32 = conn
            .query_row(
                "SELECT COUNT(*) FROM subscriptions WHERE id != 'manual'",
                [],
                |r| r.get(0),
            )
            .unwrap_or(0);
...
#[tauri::command]
pub fn get_subscriptions(state: State<'_, AppState>) -> Result<Vec<Subscription>, String> {
    let app = state.inner();
...
#[tauri::command]
pub fn toggle_subscription(state: State<'_, AppState>, id: String) -> Result<bool, String> {
    let app = state.inner();
    app.db.toggle_subscription(&id).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn refresh_all_subscriptions(state: State<'_, AppState>) -> Result<RefreshResult, String> {
    let app = state.inner();
    let subs: Vec<_> = app
        .db
    ...
    for sub in subs {
        ...
        for ep in new_eps {
            ...
            if let Some(gid) = gid {
                app.db
                    .update_episode_download_started(&episode_id, &gid)
                    .map_err(|e| e.to_string())?;
            }
...
pub fn update_auto_download(
    ...
    let app = state.inner();
    app.db
        .update_auto_download(&id, auto_download)
        .map_err(|e| e.to_string())
}
