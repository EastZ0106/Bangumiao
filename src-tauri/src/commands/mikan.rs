...
pub async fn open_mikan_browser(
    ...
    let url_str = url
        .filter(|u| !u.is_empty())
        .unwrap_or_else(|| "https://mikanani.me".into());
    let parsed_url: url::Url = url_str
        .parse()
        .map_err(|e: url::ParseError| e.to_string())?;
    let parsed_url = if is_allowed_mikan_url(&parsed_url) {
        parsed_url
    } else {
        "https://mikanani.me"
            .parse()
    ...
    let builder = WebviewBuilder::new("mikan-browser", WebviewUrl::External(parsed_url))
        .focused(true)
        .on_navigation(is_allowed_mikan_url)
...
"#,
        ...
        .on_new_window(move |_url, _features| tauri::webview::NewWindowResponse::Deny);
...
pub async fn close_mikan_browser(window: tauri::Window) -> Result<String, String> {
    ...
    if let Some(webview) = window.get_webview("mikan-browser") {
        ...
        webview
            .eval_with_callback("window.location.href", move |r| {
                let _ = tx.send(r);
            })
            .ok();
...
pub async fn mikan_eval(window: tauri::Window, js: String) -> Result<(), String> {
    let allowed = matches!(
        ...
        "history.back()"
            | "history.forward()"
            | "location.reload()"
            | "location.href='https://mikanani.me'"
    );
    if !allowed {
        return Err("unsupported mikan browser action".into());
    }
...
fn is_allowed_mikan_url(url: &url::Url) -> bool {
    matches!(
        url.host_str(),
        Some("mikanani.me") | Some("www.mikanani.me")
    )
}
...
"#;
    ...
    if let Some(webview) = window.get_webview("mikan-browser") {
        webview
            .eval_with_callback("window.__mikanScanData", move |r| {
                let _ = tx.send(r);
            })
    ...
    let combined: ScanResponse = match serde_json::from_str(&raw) {
        ...
        Err(_) => {
            ...
            let inner: String = serde_json::from_str(&raw).map_err(|e| {
                format!(
    ...
    if combined.result.is_empty() {
        let title = combined
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("?");
        let pathname = combined
            .info
            .get("pathname")
            .and_then(|v| v.as_str())
            .unwrap_or("?");
        let bangumi_id = combined
            .info
            .get("bangumiIdFromUrl")
            .and_then(|v| v.as_str())
            .unwrap_or("none");
        return Err(format!(
...
pub async fn fetch_mikan_rss(
    ...
    let feed = crate::rss_parser::parse_rss(&xml).map_err(|e| format!("RSS parse error: {}", e))?;

    let episodes: Vec<RssEpisode> = feed
        .channel
        .items
        .iter()
        .map(|item| {
...
fn loop_unwrap_json<T: serde::de::DeserializeOwned>(raw: &str) -> Result<T, String> {
    ...
    loop {
        match serde_json::from_str::<T>(&candidate) {
            Ok(v) => return Ok(v),
            Err(_) => match serde_json::from_str::<String>(&candidate) {
                Ok(inner) => {
                    candidate = inner;
                    continue;
                }
